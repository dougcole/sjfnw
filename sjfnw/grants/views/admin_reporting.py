...
from sjfnw.grants.forms import (AppReportForm, OrgReportForm,
    GPGrantReportForm, SponsoredAwardReportForm)
from sjfnw.grants.models import (GrantApplication,
    GivingProjectGrant, Organization, SponsoredProgramGrant)
from sjfnw.grants.utils import local_date_str
...
def get_app_results(options):
  ...
  apps = GrantApplication.objects.order_by('-submission_time').select_related(
  ...
  if options['report_fiscal']:
    fields += GrantApplication.fields_starting_with('fiscal')
...
def get_gpg_results(options):
  ...
  # initial queryset
  gp_awards = GivingProjectGrant.objects.select_related(
  ...
  if options.get('report_fiscal'):
    org_fields += GrantApplication.fields_starting_with('fiscal')
...
def get_sponsored_award_results(options):
  sponsored = SponsoredProgramGrant.objects.select_related('organization')
